"""
Visited screens history
=======================
Provides single shared instance of ScreensHistory class which stores and returns visited screens
...
class ScreensHistory:
    """
    Stores and returns visited screens during the current application session.
    """
    ...
    def append_screen(self, screen_name, model_id=None):
        """ Appends the active guide screen identifier to the end og the identifiers list;
            if the screen's identifier is already present in the list, it is removed
            and only the appended one is stored. """
    ...
    def remove_screen(self, screen_name, model_id):
        """ Removes screen from the history list - upon visiting the screen its previously stored identifier
